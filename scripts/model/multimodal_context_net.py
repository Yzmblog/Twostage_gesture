...
from utils.train_utils import reparameterize
...
class WavEncoder(nn.Module):
    def __init__(self):
        ...
        self.feat_extractor = nn.Sequential(
            # nn.Conv1d(64, 16, 15, stride=5, padding=1600),
            nn.Conv1d(64, 16, 16, stride=2, padding=42),
            nn.BatchNorm1d(16),
            nn.LeakyReLU(0.3, inplace=True),
            nn.Conv1d(16, 32, 5, stride=2),
            nn.BatchNorm1d(32),
            nn.LeakyReLU(0.3, inplace=True),
            nn.Conv1d(32, 64, 6, stride=1),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(0.3, inplace=True),
            nn.Conv1d(64, 32, 15, stride=2, padding=10),
        )
...
class PoseGenerator(nn.Module):
    def __init__(self, args, lang_model, pose_dim, n_words, word_embed_size, word_embeddings, z_obj=None):
        super().__init__()
    ...
    def forward(self, pre_seq, in_text, in_audio, vid_indices=None):
        ...
        if self.input_context != 'none':
            ...
            text_input = np.array(in_text)
            # print(text_input.shape)
            text_input = text_input.transpose(1, 0)
            in_text = []
            for text in text_input:
                output = []
                for word in text:
                    index = self.lang_model.get_word_index(word)
                    output.append(index)
                in_text.append(output)
            ...
            # print(audio_feat_seq.shape, text_feat_seq.shape)
