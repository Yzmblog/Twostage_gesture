...
class EmbeddingNet(nn.Module):
    def __init__(self, args, pose_dim, n_frames, n_words, word_embed_size, word_embeddings, mode):
        ...
        if mode != 'pose':
            ...
            self.pose_encoder = TemporalConvNet(num_inputs=pose_dim, num_channels=[45, 32, 32, 16])
            self.decoder = TemporalConvNet(num_inputs=16, num_channels=[16, 32, 32, 45])
        self.mode = mode
    ...
    def forward(self, in_text, in_audio, pre_poses, poses, input_mode=None, variational_encoding=False):
        ...
        if poses is not None:
            ...
            pose_mu = pose_logvar = None
            poses_feat = self.pose_encoder(poses.transpose(2, 1)).transpose(2, 1)
        ...
        out_poses = self.decoder(latent_feat.transpose(2, 1)).transpose(2, 1)
