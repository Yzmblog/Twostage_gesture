...
class Data(Modality):
  ...
  def __init__(self, path2data, speaker,
    ...
    ## Load all modalities
    self.modality_classes = self._load_modality_classes()###三个类
    ...
    self.df = self.get_df_subset('speaker', self.speaker)##取子集
  ...
  def tdt_split(self):
    ...
    ## get new train/dev/test intervals 去掉missingdata
    ...
    # h = h5py.File('/home/SENSETIME/yuzhengming/pic2.h5', 'w')
    # h.create_dataset('lenna', data=np.array(1))
    # h.close()
  ...
  def get_quantile_sample(self, data, q):
    ...
    for batch in tqdm(data, desc='quantile_sample_calc'):
      ...
      pose = pose.reshape(pose.shape[0], 2, -1).transpose(1, 2)
...
class MiniData(Dataset, HDF5):
  ...
  def __getitem__(self, idx):
    ...
    for i, modality in enumerate(self.modalities):
      ...
      if 'pose' in modality:
        pose = item[modality]
        sub_joints = self.modality_classes[modality].joint_subset
        pose = pose.reshape((pose.shape[0], 2, -1)).transpose(0, 2, 1)
        pose = pose[:, sub_joints, ]
        ...
        ###---convert to dir-----------
        ##转向量
        dir_vec = convert_pose_seq_to_dir_vec(pose)
        # normalized_dir_vec = self.normalize_dir_vec(dir_vec, self.mean_dir_vec)

        dir_vec = torch.from_numpy(dir_vec).reshape((dir_vec.shape[0], -1)).float()


        # pose = pose.transpose(0, 2, 1)
        # pose = pose.reshape((pose.shape[0], -1))

        item[modality] = dir_vec
        ###---end of convert--------------
